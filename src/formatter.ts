import { ParsedTransaction, DecodedInput } from './transaction-parser.js';
...
export function formatTransaction(
  ...
  const lines: string[] = [];
  const c = useColor ? colorize : (text: string) => text;
...
function formatParameterValue(value: DecodedInput['value']): string {
...
export function formatTransactionSummary(transactions: ParsedTransaction[]): string {
  const lines: string[] = [];
